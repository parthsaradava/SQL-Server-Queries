...
--ID is between 102 and 108. 
	...
	SELECT * FROM STUDENT
	WHERE SPI > 7 AND SPI < 9 AND STDID BETWEEN 102 AND 109
...
--28. Display details of students who belong to ‘CIVIL’ or ‘MECHANICAL’ branch and SPI is greater than 8.0. 
	...
	--DISPLAY ALL STUDENT WHOSE CITY IS RAJKOT AND BRANCH IS COMPUTER

	SELECT * FROM STUDENT
	WHERE CITY IN ('RAJKOT') AND BRANCH IN ('COMPUTER')

	--RETRIVE NAMES AND SPI OF STUDENTS WHOSE SPI IS LESS THEN 7.00

	SELECT SNAME , SPI FROM STUDENT
	WHERE SPI < 7

	--DISPLAY ALL DETAILS OF STUDENTS WHOSE ID IS  BETWEEN 103 AND 107
	
	SELECT * FROM STUDENT
	WHERE STDID BETWEEN 103 AND 107

	--RETRIVE STUDENT ID , NAME , AND BRANCH OF STUDENT WHOSE  BRANCH IS MECHANICAL

	SELECT STDID , SNAME , BRANCH FROM STUDENT
	WHERE BRANCH = 'MECHANICAL'

	--DISPLAY ALL RECORDS WHERE THE CITY IS NOT SURAT

	SELECT * FROM STUDENT
	WHERE CITY NOT IN ('SURAT')

	--DISPLAY NAMES OF STUDENTS WHOSE BRANCH IS EITHER ''ELECTRICAL' OR 'MECHANICAL'

	SELECT SNAME FROM STUDENT
	WHERE BRANCH = 'ELECTRICAL' OR BRANCH = 'MEHCANICAL'

	--RETRVIES ALL STUDENTS WHOSE SPI IS NOT BETWEEN 6 AND 8

	SELECT * FROM STUDENT
	WHERE SPI NOT BETWEEN 6 AND 8

	--DISPLAY DETAILS OF STUDENT S WHOSE CITY IS NULL
	...
	--12
	SELECT * FROM STUDENT
	WHERE CITY = 'RAJKOT' AND BRANCH NOT IN('COMPUTER')
	...
	--14
	SELECT * FROM STUDENT 
	WHERE (SPI BETWEEN 7 AND 8.5) AND (BRANCH = 'COMPUTER' OR BRANCH = 'CIVIL' OR BRANCH = 'MEHCANICAL')
